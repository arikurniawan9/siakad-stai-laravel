...
| Super Admin & operasi database | `/superadmin` | control plane privat | complete | setup akun pertama, auth dan role terisolasi, konfigurasi VA terenkripsi, backup/list/download/restore/hapus MySQL, backup final sebelum penghapusan, file session, konfirmasi sensitif, proteksi production, serta test positif-negatif tersedia; koneksi BSI riil tetap dilacak terpisah |
