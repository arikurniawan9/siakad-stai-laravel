...
final class StudentTransferController extends Controller
{
    public function preview(MasterDataImportRequest $request, MasterDataTransferService $service): RedirectResponse
    {
        ...
        $preview = $service->preview($request->file('file'), 'students');
        $request->session()->put("student_imports.{$token}", [...$preview, 'user_id' => $request->user()->id]);

        return to_route('admin.students', ['import' => $token])->with('success', 'Preview impor mahasiswa siap diperiksa.');
    }
    ...
    public function confirm(Request $request, string $token, MasterDataTransferService $service): RedirectResponse
    {
        ...
        $request->session()->forget("student_imports.{$token}");

        return to_route('admin.students')->with('success', "Impor selesai: {$result['created']} dibuat dan {$result['updated']} diperbarui.");
    }
    ...
    public function cancel(Request $request, string $token): RedirectResponse
    {
        ...
        return to_route('admin.students')->with('success', 'Preview impor dibatalkan.');
    }
    ...
    public function template(Request $request, MasterDataTransferService $service): StreamedResponse
    {
        ...
        return response()->streamDownload(fn () => $service->writeTemplate(fopen('php://output', 'wb'), 'students'), 'template-students.csv', ['Content-Type' => 'text/csv; charset=UTF-8']);
    }
    ...
    public function export(Request $request, MasterDataTransferService $service): StreamedResponse
    {
        ...
        return response()->streamDownload(fn () => $service->writeExport(fopen('php://output', 'wb'), 'students'), 'students-'.now()->format('Ymd-His').'.csv', ['Content-Type' => 'text/csv; charset=UTF-8']);
    }

    private function fromSession(Request $request, string $token): array
    {
        $preview = $request->session()->get("student_imports.{$token}");
    ...
    {
        abort_unless($request->user()->can('students.create') && $request->user()->can('students.update'), 403);
    }
}
